...
| `_workflow/templates/agents/backend-worker.template.md` | `./.claude/agents/backend-worker.md` | Phase N (N.0), if pre-selected | NOMBRE_PROYECTO, STACK_BACKEND, DOMAIN_BACKEND, CRITICAL_RULES_BACKEND | C |
| `_workflow/templates/agents/frontend-worker.template.md` | `./.claude/agents/frontend-worker.md` | Phase N (N.0), if pre-selected | NOMBRE_PROYECTO, STACK_FRONTEND, DOMAIN_FRONTEND, CRITICAL_RULES_FRONTEND | C |
| `_workflow/templates/agents/Inquisidor.template.md` | `./.claude/agents/Inquisidor.md` | Phase N (N.0), if pre-selected (opcion 1, 2 o custom) | NOMBRE_PROYECTO, TEST_DOMAIN | C |
| `_workflow/templates/agents/Sentinel.template.md` | `./.claude/agents/Sentinel.md` | Phase N (N.0), if pre-selected (opcion 1, 2 o custom) | NOMBRE_PROYECTO | C |
...
> **Sentinel-n8n** is a stack-specific preset. Not auto-installed during Phase N (N.0). User copies manually when working on n8n projects. Includes 9-phase pipeline, 11 dimensions, 6 sub-agent prompts, severity calibration, and web research protocol.
...
### Lorekeeper Config (always generated)

| Source | Destination | OW |
|--------|-------------|----|
| (generated inline, Step 3.2 step 9) | `./.claude/lorekeeper-config.json` | B |

> Note: `lorekeeper-config.json` maps logical doc roles to actual paths and thresholds. All Lorekeeper hooks and `validate-docs.sh` read this config at runtime. If config is missing or corrupt, hooks fall back to hardcoded defaults (backwards compatible). For mid-way projects, paths adapt to the existing doc structure discovered in Step 1.1.

### Version Tracking (always generated)

| Source | Destination | OW |
|--------|-------------|----|
| (generated inline, Step 3.2 step 8) | `./.claude/ignite-version.json` | B |
