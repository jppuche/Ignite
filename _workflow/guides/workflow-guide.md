...
A partir de Phase N Step N.0 (Team Assembly), los agentes estan activos con skills asignadas.
...
| Phase 1: Technical Landscape | ~10-20 min | External research + top-3 comparison + ecosystem scan |
| Phase 2: Tooling & Security | ~3-5 min per candidate | Cerbero 4-layer evaluation |
| Phase 3: Intelligence-Enriched Review | ~5-10 min | FOUNDATION.md review + decision re-evaluation |
| Phase 4: Architecture Blueprint | ~20-40 min | Design + plan hardening |
| Phase N: Development Blocks | Variable | Per-spec (includes Team Assembly at N.0) |
| Phase Final: Hardening | ~15-30 min | Audit depth dependent |

Phases 0-4 total: ~50-90 min for a typical project.
...
| **Enterprise** | 0 → 1 → 2 → 3 → 4 → N → Final | Sistemas complejos, multi-equipo, infraestructura critica |

\* Phase 3 fast-path: auto-skip si no hay catalogo + single stack + generalistas.

> **Note:** Team Assembly (formerly Phase 5) is now Step N.0 — the first sub-step of Phase N. Agent installation happens immediately before development begins.
...
- Solo Lorekeeper se instala. Demas agentes se instalan en Phase N (Step N.0: Team Assembly).
...
### Phase 1: Technical Landscape (~10-20 min)

Definir decisiones de arquitectura detalladas, herramientas de validacion y documentar decisiones.

#### 1.0.5 External Research (conditional, profile-scaled)

Before making stack decisions, enrich FOUNDATION.md with external context if the project has knowledge gaps.

**Trigger conditions** (at least one must be true):
- FOUNDATION.md was built primarily from Q&A (greenfield with sparse existing docs)
- Novel problem domain not well-covered by existing project materials
- User expressed uncertainty about approach during Discovery Q&A

**If no trigger conditions are met:** skip entirely. Log: "External research skipped — sufficient project context."

**Execution:**
- Delegate to subagent(s) for context economy (do NOT consume main context with research)
- Targeted searches on: problem domain best practices, competitive landscape, state-of-the-art approaches
- Profile scaling: Quick: skip. Standard: 1-2 searches (~3 min). Enterprise: 2-3 searches (~5 min)
- Time budget: 5 min max (strict)

**Output:** Append "External Context" appendix to FOUNDATION.md. Do NOT modify core FOUNDATION.md content — the appendix is additive context for Phase 1 decisions.

```markdown
## Appendix: External Context (Phase 1 Research)

<!-- Auto-generated by Phase 1 external research. Provides domain context for stack decisions. -->
...
#### 1.1 Stack Decisions with Top-3 Comparison

- Evaluar opciones tecnicas dentro del stack elegido (auth pattern, ORM, state management, etc.).

**For each major stack decision** (framework, ORM/data layer, auth, state management, etc.), produce a comparison matrix of top 3 options in DECISIONS.md:

```markdown
### [Decision Area]: [Selected Option]

| Option | [Dim-1] | [Dim-2] | [Dim-3] | [Dim-4] | Selected |
|--------|---------|---------|---------|---------|----------|
| Option A | ... | ... | ... | ... | * |
| Option B | ... | ... | ... | ... | |
| Option C | ... | ... | ... | ... | |

**Rationale:** [Why Option A — cite specific project requirements from FOUNDATION.md]
```

Dimensions are context-dependent (e.g., performance, ecosystem maturity, learning curve, community, TypeScript support). Select 3-5 dimensions most relevant to the project.

**Minor decisions** (formatter choice, test runner) do not require comparison matrices — a single-line entry in DECISIONS.md suffices.
...
**Exit condition:** Stack decided with comparison matrices for major decisions, validation tools defined, DECISIONS.md updated,
...
- Installation deferred to Phase N Step N.0 (assigned to specific agent)
...
### Phase 3: Intelligence-Enriched Review (~5-10 min)

> *Transparency: document every decision adjustment with the evidence that prompted it.*

Full re-evaluation enriched by installed tools from Phase 2. This review now operates on actual capabilities (installed tools) rather than candidates, making it a concrete assessment instead of speculative. Critically, this phase also re-reads FOUNDATION.md with the context of installed tools — leveraging their specialized knowledge to identify gaps and enrich the project's foundational document.

**Input:** All Phase 1 outputs (stack decisions, validation tools, DECISIONS.md, Candidate Ecosystem Catalog) + Phase 2 outputs (installed tools, rejected candidates, Tooling Summary) + docs/FOUNDATION.md.
...
#### 3.0 FOUNDATION.md Review (with installed intelligence)

Re-read FOUNDATION.md with the context of installed tools. The agent now has specialized knowledge from installed skills/MCPs that was not available during Phase 0 Discovery.

**Procedure:**
1. Re-read docs/FOUNDATION.md in full
2. For each installed tool, assess: does this tool's domain knowledge reveal gaps, risks, or opportunities not captured in FOUNDATION.md?
3. If material insights are found, propose specific updates to FOUNDATION.md
4. **User confirmation required** before any FOUNDATION.md modification — present proposed changes and ask

**Types of FOUNDATION.md updates:**
...
**If no material insights:** log "FOUNDATION.md review complete — no updates needed" in DECISIONS.md and proceed.

#### 3.1 Decision Re-evaluation
...
1. FOUNDATION.md review status (no changes / updated with user approval)
2. Confirmation or revision of each Phase 1 decision, citing installed tools as evidence
3. Agent composition recommendation (confirm pre-selection or suggest changes)
4. Architecture directives for Phase 4 (what installed tools enable or constrain)
...
```
--- Intelligence-Enriched Review Summary ---
...
**Exit condition:** User approves strategic assessment. FOUNDATION.md review completed (updated or confirmed unchanged). DECISIONS.md updated. Architecture directives documented.

### Phase 4: Architecture Blueprint (~20-40 min)
...
After creating the architecture and specs (4A), stress-test the plan before committing to execution. Errors caught here prevent hours of rework in Phase N.

**Procedure:**
1. **End-to-end walkthrough:** Walk through each spec as if executing it. Does the sequence make sense? Are dependencies clear?
2. **Dependency conflict analysis:** Identify specs that modify the same files, share data contracts, or have implicit ordering requirements. Flag conflicts.
3. **Requirement coverage check:** Verify 1:1 mapping between FOUNDATION.md requirements and specs. Every requirement must map to at least one spec. Every spec must trace back to a requirement. Flag orphans in both directions.
4. **Error handling review:** For API contracts and integration points defined in specs, verify that error paths are specified (not just happy paths).
5. **Produce Plan Integrity Report** in DECISIONS.md:

```markdown
## Plan Integrity Report (Phase 4B)

### Coverage
- FOUNDATION.md requirements: {N} → Specs: {M} (coverage: {%})
- Unmapped requirements: {list or "none"}
- Specs without requirement: {list or "none"}

### Dependencies
- Dependency conflicts identified: {N}
- Resolution: {description or "none found"}

### Risk Assessment
- High-risk specs (complex, many dependencies): {list}
- Recommended execution order: {ordered list}

### Verdict
- [ ] Ready for Phase N / [ ] Revisions needed (list)
```

**If revisions needed:** Fix within Phase 4 (return to 4A for the specific specs). Do NOT advance to Phase N with known plan defects.

**Profile gating:**
- Quick: N/A (no Phase 4)
- Standard (≤2 specs AND single stack): abbreviated — step 3 (requirement coverage check) only, produce abbreviated Plan Integrity Report
- Standard (3+ specs OR multi-stack): full hardening (all 5 steps)
- Enterprise: full hardening always

**Exit condition:** architecture.md completo, N specs en docs/specs/ cada una con exit conditions, Plan Integrity Report in DECISIONS.md with all checks passing.

### Phase N: Development Blocks

#### N.0 Team Assembly (~5-10 min)

Before the first development block, install agents and synchronize project documentation. This was formerly Phase 5 — absorbed here because it is a mechanical setup step that produces no decisions affecting later phases, and placing it immediately before development maximizes contextual relevance.

##### N.0.1 Instalar agentes pre-seleccionados
...
##### N.0.2 Review pass de CLAUDE.md y AGENT-COORDINATION.md
...
##### N.0.3 Asignar skills a agentes (including Tier 2 tools)
...
**N.0 Exit condition:** Agentes activos, AGENT-COORDINATION.md seccion 13 completa. Specs verified against installed tooling.

> **Note:** For Quick profile, N.0 is skipped (only Lorekeeper is installed, already done in Phase 0). For Standard profile, N.0 runs but without AGENT-COORDINATION.md (Teams disabled). For Enterprise, full N.0 with Teams.

#### N.1+ Development Blocks
